...
const CarDetails = () => {
    ...
    const [selectedShowroom, setSelectedShowroom] = useState(''); // Selected showroom ID
    ...
    const fetchShowrooms = (make, model, year) => {
        fetch('http://localhost:4000/api/cars/all')
            ...
            .then((data) => {
                ...
                if (filteredShowrooms.length === 1) {
                    setSelectedShowroom(filteredShowrooms[0]._id); // Automatically select if only one showroom
                }
            })
    ...
    const handleAddToCart = () => {
        ...
        addToCart(car, selectedShowroom); // Pass car and showroom ID
        setIsAdded(true);
    };
...
{reviews.length > 0 ? (
 ...
 <h3 className="text-2xl font-bold mt-10 text-gray-700">Pricing</h3>
                ...
                {showrooms.length > 0 && (
                    <div className="mt-6">
                        <h3 className="text-xl font-bold text-gray-700">Select a Showroom</h3>
                        <select
